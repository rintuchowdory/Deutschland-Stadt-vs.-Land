...
import { generateShareImage, downloadCanvas } from '../utils/shareImage.js'
...
function ScoreCircle({score,label,color}) {
  ...
  useState(() => {
    const id = requestAnimationFrame(() => setAnimDash((score/100)*c))
    ...
    <div style={{textAlign:'center'}}>
      <svg width={108} height={108} viewBox="0 0 108 108">
        ...
        <circle cx={54} cy={54} r={r} fill="none" stroke={color} strokeWidth={7}
          strokeDasharray={`${(score/100)*c} ${c}`} strokeLinecap="round" transform="rotate(-90 54 54)"
          style={{transition:'stroke-dasharray 0.6s ease'}}/>
        <text x={54} y={58} textAnchor="middle" fontSize={22} fontWeight={500} fill={color} fontFamily="Cormorant Garamond,serif">
          <CountUp value={score} />
        </text>
      </svg>
...
export default function Vergleich() {
  ...
  function handleShare() {
    const canvas = generateShareImage({ cityName: city.name, landName: land.name, cityScore: cs.total, landScore: ls.total })
    downloadCanvas(canvas, `${city.id}-vs-${land.id}.png`)
  }

  async function handleCopyLink() {
    const url = `${window.location.origin}${window.location.pathname}#/vergleich?stadt=${cityId}&land=${landId}`
    try {
  ...
  return (
    <div style={S.wrap}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',flexWrap:'wrap',gap:12,marginBottom:'2rem'}}>
        <div>
          <div style={S.eyebrow}>Direkter Vergleich</div>
          <h1 style={{fontFamily:'Cormorant Garamond',fontSize:'2.6rem',fontWeight:500,color:'var(--ink)'}}>Region vergleichen</h1>
        </div>
        <div style={{display:'flex',gap:8}}>
          <button onClick={handleCopyLink} className="btn-outline" style={{cursor:'pointer',border:'1px solid var(--ink)',fontSize:12,padding:'0.5rem 1rem'}}>
            {copied ? '✓ Kopiert' : '🔗 Link teilen'}
          </button>
          <button onClick={handleShare} className="btn-primary" style={{cursor:'pointer',border:'none',fontSize:12,padding:'0.5rem 1rem'}}>
            📥 Als Bild teilen
          </button>
        </div>
      </div>
      ...
      {/* Metrik-Tabelle */}
      <div style={{background:'white',border:'1px solid var(--mid)',overflowX:'auto'}}>
                  ...
                  <td style={{padding:'14px 16px'}}>
                    <div style={{display:'flex',height:4,gap:1}}>
                      <div style={{flex:1,background:'var(--mid)',borderRadius:1,overflow:'hidden',display:'flex',justifyContent:'flex-end'}}>
                        <div style={{width:cityW+'%',background:'var(--city)',height:'100%',borderRadius:1,transition:'width 0.5s ease'}}/>
                      </div>
                      <div style={{flex:1,background:'var(--mid)',borderRadius:1,overflow:'hidden'}}>
                        <div style={{width:landW+'%',background:'var(--land)',height:'100%',borderRadius:1,transition:'width 0.5s ease'}}/>
                      </div>
                    </div>
                  </td>
